...
save_map(struct game *cur_game)
{
	...
	for (map = 0; map < cur_game->num_maps; map++) {
		...
		fprintf(fp, "start_face=%d\n", cur_game->maps[map].start_face);
		fprintf(fp, "end_face=%d\n", cur_game->maps[map].end_face);
		/* Output map */
...
load_map(struct game *cur_game)
{
	...
	for (map = 0; map < cur_game->num_maps; map++) {
		...
		fscanf(fp, "sprite=%d\n", &cur_game->maps[map].sprite);
		fscanf(fp, "start_face=%d\n", &cur_game->maps[map].start_face);
		fscanf(fp, "end_face=%d\n", &cur_game->maps[map].end_face);
		/* input map */
		for (i = 0; i < rows; i++) {
			for (j = 0; j < cols; j++) {
...
save_char(struct user *cur_user)
{
	...
	/* Stats */
	fprintf(fp, "stats_0=%d,%d,%d\n", cur_user->character->cur_stats.life, cur_user->character->max_stats.life, cur_user->character->mod_stats.life);
	fprintf(fp, "stats_1=%d,%d,%d\n", cur_user->character->cur_stats.stamina, cur_user->character->max_stats.stamina, cur_user->character->mod_stats.stamina);
	fprintf(fp, "stats_2=%d,%d,%d\n", cur_user->character->cur_stats.magic, cur_user->character->max_stats.magic, cur_user->character->mod_stats.magic);
	fprintf(fp, "stats_3=%d,%d\n", cur_user->character->cur_stats.experience, cur_user->character->max_stats.experience);
	fprintf(fp, "stats_4=%d,%d,%d\n", cur_user->character->cur_stats.attack, cur_user->character->max_stats.attack, cur_user->character->mod_stats.attack);
	fprintf(fp, "stats_5=%d,%d,%d\n", cur_user->character->cur_stats.defense, cur_user->character->max_stats.defense, cur_user->character->mod_stats.defense);
	fprintf(fp, "stats_6=%d,%d,%d\n", cur_user->character->cur_stats.dodge, cur_user->character->max_stats.dodge, cur_user->character->mod_stats.dodge);
	fprintf(fp, "stats_7=%d,%d,%d\n", cur_user->character->cur_stats.power, cur_user->character->max_stats.power, cur_user->character->mod_stats.power);
	fprintf(fp, "stats_8=%d,%d,%d\n", cur_user->character->cur_stats.spirit, cur_user->character->max_stats.spirit, cur_user->character->mod_stats.spirit);
	fprintf(fp, "stats_9=%d,%d,%d\n", cur_user->character->cur_stats.avoid, cur_user->character->max_stats.avoid, cur_user->character->mod_stats.avoid);	
...
load_char(struct user *cur_user)
{
	...
	/* Stats */
	fscanf(fp, "stats_0=%d,%d,%d\n", &cur_user->character->cur_stats.life, &cur_user->character->max_stats.life, &cur_user->character->mod_stats.life);
	fscanf(fp, "stats_1=%d,%d,%d\n", &cur_user->character->cur_stats.stamina, &cur_user->character->max_stats.stamina, &cur_user->character->mod_stats.stamina);
	fscanf(fp, "stats_2=%d,%d,%d\n", &cur_user->character->cur_stats.magic, &cur_user->character->max_stats.magic, &cur_user->character->mod_stats.magic);
	fscanf(fp, "stats_3=%d,%d\n", &cur_user->character->cur_stats.experience, &cur_user->character->max_stats.experience);
	fscanf(fp, "stats_4=%d,%d,%d\n", &cur_user->character->cur_stats.attack, &cur_user->character->max_stats.attack, &cur_user->character->mod_stats.attack);
	fscanf(fp, "stats_5=%d,%d,%d\n", &cur_user->character->cur_stats.defense, &cur_user->character->max_stats.defense, &cur_user->character->mod_stats.defense);
	fscanf(fp, "stats_6=%d,%d,%d\n", &cur_user->character->cur_stats.dodge, &cur_user->character->max_stats.dodge, &cur_user->character->mod_stats.dodge);
	fscanf(fp, "stats_7=%d,%d,%d\n", &cur_user->character->cur_stats.power, &cur_user->character->max_stats.power, &cur_user->character->mod_stats.power);
	fscanf(fp, "stats_8=%d,%d,%d\n", &cur_user->character->cur_stats.spirit, &cur_user->character->max_stats.spirit, &cur_user->character->mod_stats.spirit);
	fscanf(fp, "stats_9=%d,%d,%d\n", &cur_user->character->cur_stats.avoid, &cur_user->character->max_stats.avoid, &cur_user->character->mod_stats.avoid);
